...
wide = pct.pivot_table(index="company_id", columns="metric", values="value", aggfunc="first")
wide = wide.merge(companies[["id", "company_name"]], left_index=True, right_on="id", how="left")
...
AXES = ["return_on_equity_pct", "operating_profit_margin_pct", "net_profit_margin_pct",
...
if axes_present and selected_company in wide.index:
    ...
    fig.add_trace(go.Scatterpolar(r=peer_avg + [peer_avg[0]],
                                  theta=axes_present + [axes_present[0]],
                                  name=f"{group} average", line=dict(dash="dash")))
...
try:
    ...
    conn.close()
    group_col = next((c for c in peers_raw.columns
