...
def validate(tables: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    if bs is not None and {"total_assets", "total_liabilities"}.issubset(bs.columns):
        ...
        for idx in bs.index[diff > 0.01]:
            _add(
                rows,
                bs.at[idx, "company_id"],
                bs.at[idx, "year"],
                "balancesheet.total_assets",
                "BS does not balance >1%",
                WARNING,
            )

    # DQ-05 OPM cross-check |opm - op/sales*100| < 1 (WARNING)
    if pl is not None and {"opm_percentage", "operating_profit", "sales"}.issubset(
        pl.columns
    ):
    ...
    if cf is not None and {
        ...
        s = (
            pd.to_numeric(cf["operating_activity"], errors="coerce")
        ...
        for idx in cf.index[(net - s).abs() > 10]:
            _add(
                rows,
                cf.at[idx, "company_id"],
                cf.at[idx, "year"],
                "cashflow.net_cash_flow",
                "Net cash != CFO+CFI+CFF (>10 Cr)",
                WARNING,
            )
    ...
    if docs is not None and "Annual_Report" in docs.columns:
        for idx in docs.index[
            docs["Annual_Report"].isna()
            | (docs["Annual_Report"].astype(str).str.strip() == "")
        ]:
            _add(
                rows,
                docs.at[idx, "company_id"],
                docs.at[idx, "Year"] if "Year" in docs.columns else None,
            ...
            for cid, n in counts.items():
                if n < 5:
                    _add(
                        ...
                        None,
                        f"{name}.coverage",
                        f"Only {n} yrs of {name} (<5)",
                        WARNING,
                    )

    return pd.DataFrame(
        rows, columns=["company_id", "year", "field", "issue", "severity"]
    )


def run_and_save(
    tables: dict[str, pd.DataFrame],
    out_path: str | Path = "output/validation_failures.csv",
) -> pd.DataFrame:
