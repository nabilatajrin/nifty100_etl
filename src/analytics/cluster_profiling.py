...
def profile_clusters(
    fr_latest: pd.DataFrame, cluster_labels: pd.DataFrame
) -> pd.DataFrame:
...
def correlation_heatmap(
    fr_latest: pd.DataFrame, out_path: str = "reports/correlation_heatmap.png"
):
...
def detect_outliers(
    fr_latest: pd.DataFrame, sectors: pd.DataFrame, z_threshold: float = 3.0
) -> pd.DataFrame:
...
def main():
    ...
    fr_latest = (
        fr.sort_values("year").groupby("company_id").tail(1).reset_index(drop=True)
    )
    ...
    if not cluster_path.exists():
        raise FileNotFoundError(
            "output/cluster_labels.csv not found — run Day 36 (clustering.py) first."
        )
