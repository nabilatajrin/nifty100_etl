...
def con_03_opm_declining_3yr(h: pd.DataFrame):
    if len(h) < 3:
        return None
    if (
        _consecutive_trend(h["operating_profit_margin_pct"], 3, improving=False)
        and h["operating_profit_margin_pct"].tail(3).iloc[0]
        > h["operating_profit_margin_pct"].tail(3).iloc[-1]
    ):
        return (
            75,
...
def con_11_net_debt_high(h: pd.DataFrame):
    ...
    if (
        pd.notna(debt)
        and pd.notna(ebitda_proxy)
        and ebitda_proxy > 0
        and debt > 3 * ebitda_proxy
    ):
        return (
            65,
...
CON_RULES_RATIO = [
    con_02_fcf_negative_3yr,
    con_03_opm_declining_3yr,
    con_04_net_loss,
    con_06_icr_low,
    con_07_payout_over_100,
    con_08_de_rising_3yr,
    con_09_eps_declining_3yr,
    con_10_roce_low,
    con_11_net_debt_high,
...
def generate_for_company(
    cid: str, ratio_h: pd.DataFrame, bs_h: pd.DataFrame, pl_h: pd.DataFrame, sector: str
) -> list:
...
def main():
    ...
    out = pd.DataFrame(
        all_entries, columns=["company_id", "type", "rule_id", "text", "confidence_pct"]
    )
