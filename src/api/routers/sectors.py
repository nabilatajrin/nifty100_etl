...
def list_sectors():
    ...
    conn = get_connection()
    sectors = [r["broad_sector"] for r in
              conn.execute("SELECT DISTINCT broad_sector FROM sectors WHERE broad_sector IS NOT NULL")]
    ...
    for sector in sectors:
        ...
        roes = sorted(r["return_on_equity_pct"] for r in med if r["return_on_equity_pct"] is not None)
        des = sorted(r["debt_to_equity"] for r in med if r["debt_to_equity"] is not None)
...
def sector_companies(sector: str):
    ...
    conn = get_connection()
    exists = conn.execute("SELECT 1 FROM sectors WHERE broad_sector = ? LIMIT 1",
                          (sector,)).fetchone()
    if not exists:
        conn.close()
