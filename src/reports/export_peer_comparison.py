...
def write_sheet(
    ws, wide: pd.DataFrame, companies: pd.DataFrame, benchmark_company: str | None
):
    ...
    cols = ["company_id", "company_name"] + [
        c for pair in metric_cols for c in (pair, f"{pair}_pctile") if c in wide.columns
    ]
    ...
    for j, col in enumerate(cols, 1):
        if col in wide.columns and col not in ("company_id", "company_name"):
            ...
            if pd.notna(median_val):
                ws.cell(row=summary_row, column=j, value=round(median_val, 2)).font = (
                    BOLD_FONT
                )
