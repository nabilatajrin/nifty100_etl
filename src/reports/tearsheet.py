...
KPI_LABEL = ParagraphStyle(
    "kpi_label", parent=styles["Normal"], fontSize=9, textColor=colors.white
)
...
def _roe_roce_chart(ratio_h: pd.DataFrame) -> Image:
    ...
    if "return_on_capital_employed_pct" in hist.columns:
        ax.plot(
            hist["year"],
            hist["return_on_capital_employed_pct"],
            marker="s",
            label="ROCE %",
            color="#C00000",
        )
...
def _balance_sheet_chart(bs_h: pd.DataFrame) -> Image:
    ...
    ax.bar(
        hist["year"],
        other,
        bottom=equity + hist["borrowings"].fillna(0),
...
def _cashflow_waterfall(cf_latest: pd.Series) -> Image:
    ...
    vals = [
        cf_latest.get("operating_activity", 0) or 0,
...
def build_tearsheet(
    ...
    # ---- KPI tiles ----
    latest = (
        ratio_h.sort_values("year").iloc[-1]
    ...
    # Pros / Cons
    pros_style = ParagraphStyle(
        "pros", parent=styles["Normal"], textColor=GREEN, fontSize=9
    )
    cons_style = ParagraphStyle(
        "cons", parent=styles["Normal"], textColor=RED, fontSize=9
    )
    ...
    badge = Table(
        [
            [
                Paragraph(
                    f"Capital Allocation: {capital_label or 'N/A'}",
                    ParagraphStyle(
                        "badge",
                        textColor=colors.white,
                        fontSize=11,
...
def _test_run():
    ...
    for ticker in test_tickers:
        comp = pd.read_sql(
            "SELECT company_name FROM companies WHERE id=?", conn, params=(ticker,)
        )
        if comp.empty:
