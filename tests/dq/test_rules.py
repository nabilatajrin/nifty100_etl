    ...
    cf = pd.DataFrame(
        {
            "company_id": ["TCS"] * 5,
            "year": years,
            "operating_activity": [30] * 5,
            "investing_activity": [-10] * 5,
            "financing_activity": [-15] * 5,
            "net_cash_flow": [5] * 5,
        }
    )
...
def test_dq01_duplicate_company_pk():
    ...
    dq01 = f[
        (f["severity"] == "CRITICAL")
        & (f["issue"].str.contains("company PK", na=False))
    ]
...
def test_dq16_insufficient_coverage():
    ...
    t["profitandloss"] = pl[
        ~(
            (pl["company_id"] == "TCS")
            & (pl["year"].isin(["2022-03", "2023-03", "2024-03"]))
        )
    ].copy()
    f = validate(t)
