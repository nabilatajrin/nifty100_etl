...
def test_db(tmp_path_factory):
    ...
    pd.DataFrame(
        {
            "id": ids,
            "company_name": [f"Company {i}" for i in range(len(ids))],
            "roe_percentage": [20.0] * len(ids),
            "roce_percentage": [18.0] * len(ids),
        }
    ).to_sql("companies", conn, index=False)
    pd.DataFrame(
        {
            "company_id": ids,
            "broad_sector": sectors_list,
            "sub_sector": "x",
            "market_cap_category": "Large Cap",
        }
    ).to_sql("sectors", conn, index=False)
    ...
    for t in ["profitandloss", "balancesheet", "cashflow"]:
        pd.DataFrame({"company_id": ids, "year": "2024-03"}).to_sql(
            t, conn, index=False
        )
    ...
    pd.DataFrame({"company_id": [], "pros": []}).to_sql(
        "prosandcons", conn, index=False
    )
    pd.DataFrame({"company_id": [], "pe_ratio": []}).to_sql(
        "market_cap", conn, index=False
    )
